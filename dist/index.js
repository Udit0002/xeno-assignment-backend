...
app.post("/api/tenants/onboard", async (req, res) => {
    ...
    try {
        ...
        if (ngrokUrl) {
            try {
                const results = await registerWebhooks(tenant.shop, tenant.accessToken, ngrokUrl);
...
app.get("/api/:tenantId/orders/by-date", async (req, res) => {
    try {
        ...
        if (startParam || endParam) {
            where.createdAt = {};
            if (startParam)
                where.createdAt.gte = new Date(startParam + "T00:00:00.000Z");
            if (endParam)
                where.createdAt.lte = new Date(endParam + "T23:59:59.999Z");
        }
