 ...
 */
async function fetchShopifyCustomerById(client: any, shopifyCustomerId: string) {
  try {
 ...
 */
export async function fullSync(tenantId: string) {
  const tenant = await prisma.tenant.findUnique({ where: { id: tenantId } });
  if (!tenant) throw new Error("tenant not found");
  if (!tenant.accessToken || !tenant.shop) throw new Error("tenant missing accessToken or shop");
  ...
  for (const p of products) {
    try {
      ...
      await prisma.product.upsert({
        where: { tenantId_shopifyProductId: { tenantId, shopifyProductId } as any },
        update: {
  ...
  for (const c of customers) {
    try {
      ...
      try {
        ...
        if (existing) await prisma.customer.update({ where: { id: existing.id }, data });
        else await prisma.customer.create({ data: { tenantId, shopifyCustomerId, ...data } });
      }
      customerCount++;
    } catch (e) {
  ...
  for (const o of orders) {
    try {
      ...
      await prisma.order.upsert({
        where: { tenantId_shopifyOrderId: { tenantId, shopifyOrderId } as any },
        update: op,
  ...
  for (const row of missing) {
    ...
    if (!row.shopifyCustomerId) continue;
    try {
      ...
      if (shopifyCustomer) {
        ...
        if (shopifyCustomer.email && shopifyCustomer.email !== row.email) updateData.email = shopifyCustomer.email;
        if (shopifyCustomer.first_name && shopifyCustomer.first_name !== row.firstName) updateData.firstName = shopifyCustomer.first_name;
        if (shopifyCustomer.last_name && shopifyCustomer.last_name !== row.lastName) updateData.lastName = shopifyCustomer.last_name;
